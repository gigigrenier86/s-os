...
varrun="/run/s-android-lxc"
varlib="/var/lib"
net_link_key="lxc.net.0.link"
case "$(lxc-info --version)" in [012].*) net_link_key="lxc.network.link" ;; esac
vnic=$(awk "\$1 == \"$net_link_key\" {print \$3}" /var/lib/waydroid/lxc/android/config)
: ${vnic:=s-android0}

if [ "$vnic" != "s-android0" ]; then
...
start() {
    ...
    # Le fichier temoin seul ne suffit pas : mesure le 2026-08-28, il
    # survit dans /run pendant que dnsmasq, lui, meurt a chaque
    # `systemctl stop s-android.service` (meme cgroup que lxc-start). Sans
    # ce controle, un redemarrage "voit" le reseau deja monte et ne relance
    # jamais dnsmasq -- le pont existe, aucune IP n'est jamais distribuee.
    if [ -f "${varrun}/network_up" ] && [ -f "${varrun}/dnsmasq.pid" ] \
       && kill -0 "$(cat "${varrun}/dnsmasq.pid" 2>/dev/null)" 2>/dev/null; then
        echo "waydroid-net is already running"
        exit 0
    fi
